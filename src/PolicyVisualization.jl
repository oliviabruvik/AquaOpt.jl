...
# ╔═╡ fee34b0e-5c3e-11f0-20d2-8be2d76d25fc
begin
	using PlutoUI

	md"""
	# Aquatech Optimization
	##### Exploring QMDP, SARSOP, and MDP with assumed true distribution

	Built on Robert Moss' Julia Academy: Decision Making Under Uncertainty with POMDPs.jl.
	"""
end

# ╔═╡ 57876732-8a34-4545-8f41-962af1a55556
using POMDPs, QuickPOMDPs, POMDPModelTools, BeliefUpdaters, Parameters

# ╔═╡ aa223780-ad0e-42f1-8d82-66116302b315
using POMDPPolicies, POMDPTools, POMDPModels, POMDPSimulators, POMDPLinter

# ╔═╡ c6a4ab02-f49b-4d05-a240-d4a0d2658997
using QMDP, DiscreteValueIteration, LocalFunctionApproximation

# ╔═╡ 1963a915-9a5b-424d-88eb-e0a12a35a2eb
using NativeSARSOP: SARSOPSolver

# ╔═╡ 5ba33a47-169f-40a2-b043-f889b074d8f1
using DataFrames, CSV, JLD2, Logging, Optim

# ╔═╡ 27222db1-c291-4a69-9428-9566a43db15c
using Discretizers, Distributions, GaussianFilters, Random, Statistics

# ╔═╡ eff5d96e-aceb-49c8-a625-d2fad031c669
using Plots: plot, plot!, scatter, scatter!, heatmap, heatmap!, histogram, histogram!, savefig

# ╔═╡ 103acb14-aee2-4d7d-a033-d9635a299bea
using PlotlyJS

# ╔═╡ 0bc9385e-4bbf-4bda-836a-07c1d645b295
include("../src/Algorithms/Simulation.jl")

# ╔═╡ fc1be598-cb1e-40eb-9839-1bc95cfd60da
include("../src/Utils/Config.jl")

# ╔═╡ b19e6487-17bf-4dd7-ac29-40050c69d417
include("../src/Plotting/Comparison.jl")

# ╔═╡ c13b6b72-aaf6-4dcd-8e10-e10b4e4a99a6
include("../src/Plotting/Heatmaps.jl")

# ╔═╡ bc2eb064-a6d2-4bd7-92fc-78bf2846096b
include("../src/Plotting/Timeseries.jl")

# ╔═╡ 8b6f9663-930b-45a1-9abc-cb173e2fc8f2
md"""
## Notebook Outline
- Data cleaning
- SARSOP
- QMDP
- MDP with assumed true distribution
"""

# ╔═╡ 34166443-e4dc-42e4-aa11-020a01b6d2af
md"""
## Problem Definition
...
:----------- | :-------------- | :------:
$\mathcal{S}$ | State space | Sea lice level (Discrete to 1 DP)
$\mathcal{A}$ | Action space | Treatment or NoTreatment
$T$ | Transition function | $T(s^\prime \mid s, a) = (1-\rho)e^{r} \cdot s.SeaLiceLevel$
$R$ | Reward function | $R(s,a) = - (\lambda s.SeaLiceLevel + (1 - \lambda) costOfTreatment)$
$\gamma \in [0,1]$ | Discount factor | $\gamma = 0.95$

We also have a model in log space:

Variable | Description | Models
:----------- | :-------------- | :------:
$\mathcal{S}$ | State space | Log sea lice level (Discrete to 1 DP)
$\mathcal{A}$ | Action space | Treatment or NoTreatment
$T$ | Transition function | $T(s^\prime \mid s, a) = log(1-\rho) + r + s.LogSeaLiceLevel$
$R$ | Reward function | $R(s,a) = - (\lambda e^{s.LogSeaLiceLevel} + (1 - \lambda) costOfTreatment)$
$\gamma \in [0,1]$ | Discount factor | $\gamma = 0.95$
"""

# ╔═╡ 47e8b48b-9f75-452d-b32c-95a211f29072
plotlyjs()

# ╔═╡ f7fa597a-7058-4745-9f73-698e82b63e49
md"""
## Kalman Filter
"""

# ╔═╡ 4a6a9502-0f3c-4cdd-8ca0-83e550a4ed53
md"""
## SeaLicePOMDP.jl
"""

# ╔═╡ 7d61db10-14cb-454a-b9f4-3db212597c46
begin
	# -------------------------
...
# ╔═╡ 0c61ce46-9b79-4437-b0bc-e8bf1ca9094f
md"""
## SeaLiceLogPOMDP.jl
"""

# ╔═╡ 4d6cfa04-c35f-46ff-9218-1eda85d9495d
begin
	# -------------------------
	# State, Observation, Action
	# -------------------------
...
# ╔═╡ 61951ab8-bde5-41e3-bf0a-dbf9cc4f28c0
md"""
## SimulationPOMDP.jl
"""

# ╔═╡ 33561a0c-dab4-4e34-9181-3e1056e01f05
begin
	# -------------------------
	# State, Observation, Action
	# -------------------------
...
# ╔═╡ 33c34aff-05c7-4420-8cb6-95e50cd6215c
md"""
## SimulationLogPOMDP.jl
"""

# ╔═╡ 829d39d4-5cb3-4410-91c2-d582ff68d221
begin
	# -------------------------
	# State, Observation, Action
	# -------------------------
...
# ╔═╡ 04def329-3b57-497c-803f-0c8fb05a67a2
begin
...
# ╔═╡ 2d07516c-fcb2-411c-aa3a-9ae298271298
md"""
## Evaluation.jl
"""

# ╔═╡ 822c4826-6d05-4ace-a67e-33569fc646ee
md"""
## Policies.jl
"""

# ╔═╡ a1ece528-a2c3-49ce-a1e2-5348c38096c7
begin
	# ----------------------------
	# Policy Generation
	# ----------------------------
...
# ╔═╡ 25f37518-5e8a-4aad-8ae9-ef67ea7a4fa8
begin
	# ----------------------------
	# Optimizer Wrapper
	# ----------------------------
...
# ╔═╡ b803eef0-ba0f-4bc0-90c6-60d86a7dfd4b
md"""
## Play around
"""

# ╔═╡ 06528f74-9090-4678-8171-e8c590f3ab7e
log_space = true

# ╔═╡ 8c002c3f-caf6-4aac-8cde-5a53dfa13021
CONFIG = Config(num_episodes=3, steps_per_episode=52)

# ╔═╡ 98b35b66-8bb5-43f6-98a4-0c42efc66f91
POMDP_CONFIG = POMDPConfig(log_space=log_space)

# ╔═╡ b3524adf-c550-4cfe-ab10-9ef9e83ae791
algorithms = [
	...
	Algorithm(
		...
		heuristic_belief_threshold=CONFIG.heuristic_belief_threshold, 
		heuristic_rho=CONFIG.heuristic_rho
	),
	Algorithm(
		solver=ValueIterationSolver(max_iterations=30),
		convert_to_mdp=true,
		solver_name="VI_Policy"
	),
...
# ╔═╡ da399c36-fdd9-4f17-8f52-9657b0478f89
begin
    ...
    if isfile(results_file_path)
		...
		# all_results = load(results_file_path; typemap=Dict("Main.SeaLiceLogState" => SeaLiceLogState, "Main.Action" => Action))
    else
...
# ╔═╡ 87a5becc-6ed6-434b-b9aa-7aeb7d0c406c
#all_results

# ╔═╡ 6c914a31-841a-4da7-8e4b-64b522311e6c
algo = algorithms[6]

# ╔═╡ 741bb718-a5c8-4c15-a739-a597276041f7
 results = all_results[algo.solver_name]

# ╔═╡ 63357e63-fe96-4619-94ba-e3a977464087
println(results.lambda)

# ╔═╡ 59d3f744-edc8-44f5-8e81-cffe79728dce
...
# ╔═╡ 8fa6201d-fb6a-4ec8-80ad-811748709586
begin
	title = algo.solver_name
...
# ╔═╡ 6e432d30-3146-4e43-82e6-b82b50c18cc8
println(mean_costs)

# ╔═╡ 72a06f15-817a-4f5b-b483-f1bbf89a2ead
plot_policy_cost_vs_sealice(results, algo.solver_name, CONFIG, POMDP_CONFIG)

# ╔═╡ abf86f1b-f1d6-46c7-8eba-375f053d3e48


# ╔═╡ 3d5ddb17-2420-4223-86d7-01561d12661d
...
GaussianFilters = "~0.1.3"
JLD2 = "~0.5.13"
LocalFunctionApproximation = "~1.1.2"
NativeSARSOP = "~0.1.3"
Optim = "~1.12.0"
POMDPLinter = "~0.1.2"
...
manifest_format = "2.0"
project_hash = "1f5b0caa3e4ef507e114f56951198908ea92a360"
...
[[deps.GaussianFilters]]
deps = ["Distributions", "ForwardDiff", "LinearAlgebra", "Logging", "Random"]
git-tree-sha1 = "a3586c426fc4dd3cca8d1482f4dd495afc0897a9"
...
git-tree-sha1 = "7c3c289115bf819cc2d9cd2b7a756d510eb85395"
